<?php
    ...
    */
    ...
    if ($_SERVER['HTTP_HOST'] != 'localhost') {
        ...
        $objEmailMessage->Subject = 'Exception on ' . $_SERVER['HTTP_HOST'];
        ob_start();
?>
<html>
    <body>
        <p>
            <span><?php _p($__exc_strType); ?> in PHP Script<br /></span><?php _p($_SERVER["PHP_SELF"]); ?>
        </p>
         <p>
        ...
        <div>
            <span><?php _p($__exc_strMessageBody, false); ?></span><br />
                ...
                <b>Username:</b> <?php _p(QApplication::$objUser->Username) ?> <br />
                <b>Email:</b> <?php _p(QApplication::$objUser->Email) ?> <br />
    ...
    <?php
                            ...
                            for ($__exc_intLine = max(1, $__exc_intLineNumber - 5); $__exc_intLine <= min(count($__exc_objFileArray), $__exc_intLineNumber + 5); $__exc_intLine++) {
                                if ($__exc_intLineNumber == $__exc_intLine)
                                    printf("<i>Line %s:    %s</i>", $__exc_intLine, htmlentities($__exc_objFileArray[$__exc_intLine - 1]));
                                else
...
<?php
    $objEmailMessage->HtmlBody = ob_get_contents();

    $objEmailMessage->Body = strip_tags(str_replace('<br />', "\n", $objEmailMessage->HtmlBody));

    //mail($objEmailMessage->To, $objEmailMessage->Subject, $__exc_strType . "\n" . $__exc_strMessage . "\n" . $__exc_strObjectType . "\n" . $__exc_strFilename . "\n" . $__exc_intLineNumber . "\n" . $__exc_strStackTrace . "\n");
    //mail($objEmailMessage->To, $objEmailMessage->Subject, $objEmailMessage->Body);
