    ...
    function extract_files()
    {
        ...
        if ($fp = $this->open_archive())
        {
            ...
            while ($block = fread($fp, 512))
            {
                ...
                {
                    $this->error[] = "Could not open {$file['name']} for writing in " . getcwd();
                    continue;
                }
...
class zip_file extends archive
{
    ...
    function extract_files() {
        ...
        if ($zip->open($this->options['name'])!==TRUE) {
    ...
    function create_zip() {
        ...
        $zip = new ZipArchive();
        if ($zip->open($this->options['basedir'] . '/' . $this->options['name'], ZIPARCHIVE::OVERWRITE)!==TRUE) {
            throw new Exception(__METHOD__ . ':Can\'t open file: ' . $this->options['basedir'] . '/' .$this->options['name']);
        }
