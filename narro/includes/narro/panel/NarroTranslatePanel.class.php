    ...
    class NarroTranslatePanel extends QPanel {
        ...
        public function txtTranslation_Focus($strFormId, $strControlId, $strParameter) {
            foreach($this->dtrText->GetChildControls() as $ctl) {
                if ($ctl instanceof NarroContextInfoEditor) {
                    if ($ctl->Translation->ControlId != $strControlId && $ctl->Changed) {
                        $ctl->btnSave_Click($strFormId, $strControlId, $strParameter);
                        if ($ctl->Translation->Warning) {
                            $ctl->Translation->Focus();
                            break;
                        }
                    }
        ...
        protected function dtrText_Conditions($blnReset = false) {
            ...
            $this->arrClauses = array(
                QQ::LimitInfo($this->intMaxRowCount+=10, 0),
            ...
            if ($this->txtFile->Text)
                if (preg_match("/^'.*'$/", $this->txtFile->Text))
                    $this->arrConditions[] = QQ::Equal(QQN::NarroContextInfo()->Context->File->FileName, substr($this->txtFile->Text, 1, -1));
                else
                    $this->arrConditions[] = QQ::Like(QQN::NarroContextInfo()->Context->File->FileName, '%' . $this->txtFile->Text . '%');
