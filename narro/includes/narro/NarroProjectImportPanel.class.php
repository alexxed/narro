<?php
    ...
    class NarroProjectImportPanel extends QPanel {
        ...
        public function __construct($objNarroProject, $objParentObject, $strControlId = null) {
            ...
            $this->strTemplate = __DOCROOT__ . __SUBDIRECTORY__ . '/templates/NarroProjectImportPanel.tpl.php';
            ...
            if (NarroUtils::IsProcessRunning('import', $this->objNarroProject->ProjectId)) {
                ...
                NarroApp::ExecuteJavaScript(sprintf('lastImportId = setInterval("qcodo.postAjax(\'%s\', \'%s\', \'QClickEvent\', \'1\');", %d);', $this->Form->FormId, $this->btnImport->ControlId, 2000));
            }
        ...
        public function btnImport_Click($strFormId, $strControlId, $strParameter) {
            ...
            elseif ($strParameter == 2) {
                ...
                try {
                    $strCommand = sprintf(
                        __PHP_CLI_PATH__ . ' ' .
                            ...
                            (($this->chkImportOnlyTranslations->Checked || !NarroApp::HasPermission('Can import project', $this->objNarroProject->ProjectId))?'--only-suggestions --do-not-deactivate-files --do-not-deactivate-contexts ':'') .
                            ' --template-lang en-US --translation-lang %s --template-directory "%s" --translation-directory "%s"',
                ...
                if ($mixProcess) {
                    if (NarroApp::$UseAjax)
                        NarroApp::ExecuteJavaScript(sprintf('lastImportId = setInterval("qc.pA(\'%s\', \'%s\', \'QClickEvent\', \'1\')", %d);', $strFormId, $strControlId, 2000));
                    else
                     ...
                     */
                    if (NarroApp::$UseAjax)
                        NarroApp::ExecuteJavaScript(sprintf('lastImportId = setTimeout("qc.pA(\'%s\', \'%s\', \'QClickEvent\', \'2\')", %d);', $strFormId, $strControlId, 2000));
                    else
