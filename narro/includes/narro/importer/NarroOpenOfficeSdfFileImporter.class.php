    ...
    class NarroOpenOfficeSdfFileImporter extends NarroFileImporter {
        ...
        public function ExportSdfFile($intProjectId, $strTemplateFile, $strOutputFile) {
            ...
            if (!$hndTranslatedFile) {
                NarroLog::LogMessage(3, __LINE__ . ':' . sprintf('Nu se poate deschide fișierul „%s” pentru citire', $strOutputFile));
                return false;
            }

            $this->intTotalToProcess = count(file($strTemplateFile));

            NarroLog::LogMessage(1, __LINE__ . ':' . sprintf('Începe procesarea fișierului „%s” (%d texte), rezultatul se va scrie în „%s”.', $strTemplateFile, $this->intTotalToProcess, $strOutputFile));
...
//            }
            ...
            while(!feof($hndTemplateFile)) {
                ...
                if ($this->intProcessedSoFar % 1000 == 0)
                    NarroLog::LogMessage(1, __LINE__ . ':' . 'Progres: ' . floor(($this->intProcessedSoFar * 100)/$this->intTotalToProcess) . "%");
        ...
        public function ImportSdfFile($intProjectId, $strTemplateLang, $strTranslationLang = null, $strTemplateFile, $blnCheckEqual = false, $blnValidate = false, $blnOnlySuggestions = false) {
            ...
            while(!feof($hndFile)) {
                ...
                if (!$strTranslationLang && $strLangCode != $strTemplateLang) {
                    NarroLog::LogMessage(2, sprintf(t('Skipped line "%s" because the language code found "%s" does not match the one passed in the arguments, "%s"'), $strFileLine, $strLangCode, $strTemplateLang));
                    continue;
                }
                ...
                if ($strTranslationLang) {
                    ...
                    if (
                        ...
                        NarroLog::LogMessage(2, sprintf(t('Original line and translation line do not match. Here is the condition checked: "%s". Skipping.'), $arrColumn[0] .'=='.  $arrTranslatedColumn[0] . " && " .
                        $arrColumn[1] .'=='.  $arrTranslatedColumn[1] . " && " .
                        $arrColumn[3] .'=='.  $arrTranslatedColumn[3] . " && " .
                        $arrColumn[4] .'=='.  $arrTranslatedColumn[4] . " && " .
                        $arrColumn[5] .'=='.  $arrTranslatedColumn[5] . " && " .
                ...
                if (preg_match_all('/([^\.\\\]+)[\\\\.]{1,1}/', $arrColumn[1], $arrColMatches)) {
                    ...
                    foreach($arrColMatches[1] as $intKey=>$strFileName) {
                        ...
                        if (!isset($arrFiles[$strPath . '/' . $strFileName])) {
                            ...
                            if (!$objFile instanceof NarroFile) {
                                ...
                                $objFile->Save();
                                NarroLog::LogMessage(1, sprintf('S-a adăugat pseudo fișierul „%s” cu calea „%s”', $strFileName, $strPath));
                    ...
                    NarroLog::LogMessage(3, $arrColumn[1] . ' faile preg_match');
                }
                ...
                if ($this->intProcessedSoFar % 100 == 0 && floor(($this->intProcessedSoFar * 100)/$this->intTotalToProcess) != $intLastPercent) {
                    NarroLog::LogMessage(1, 'Progres: ' . floor(($this->intProcessedSoFar * 100)/$this->intTotalToProcess) . "%");
            ...
            fclose($hndFile);
            $this->stopTimer();
            NarroLog::LogMessage(1, 'Import terminat.');
            NarroLog::LogMessage(1, var_export(NarroImportStatistics::$arrStatistics,true));
        }
    }
?>
