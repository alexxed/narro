<?php
	 ...
	 * listing a collection of NarroUserRole objects.  It includes
	 ...
	 *
	 */
	class NarroUserRoleDataGridGen extends QDataGrid {
		 ...
		 * way of adding a column to this NarroUserRole datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the NarroUserRole object, or it can be any QQNode extending from QQN::NarroUserRole().
		 *
		 ...
		 * sorting using two fields from the NarroUserRole object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from NarroUserRole
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= (strpos($strLinkUrl, '?') !== false ? '&' : '?').'intUserRoleId=<?=urlencode($_ITEM->UserRoleId)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=urlencode($_ITEM->UserRoleId)?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->UserRoleId, false); ?>>' . $strLinkHtml . '</a>';
			$colEditColumn = new QDataGridColumn($strColumnTitle, $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a NarroUserRole-based QQNode.
		 * It will also verify that it is a proper NarroUserRole-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'narro_user_role') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "narro_user_role".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'UserRoleId': return QQN::NarroUserRole()->UserRoleId;
				case 'UserId': return QQN::NarroUserRole()->UserId;
				case 'User': return QQN::NarroUserRole()->User;
				case 'RoleId': return QQN::NarroUserRole()->RoleId;
				case 'Role': return QQN::NarroUserRole()->Role;
				case 'ProjectId': return QQN::NarroUserRole()->ProjectId;
				case 'Project': return QQN::NarroUserRole()->Project;
				case 'LanguageId': return QQN::NarroUserRole()->LanguageId;
				case 'Language': return QQN::NarroUserRole()->Language;
				default: throw new QCallerException('Simple Property not found in NarroUserRoleDataGrid content: ' . $mixContent);
