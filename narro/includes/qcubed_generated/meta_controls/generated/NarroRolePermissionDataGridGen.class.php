<?php
	 ...
	 * listing a collection of NarroRolePermission objects.  It includes
	 ...
	 *
	 */
	class NarroRolePermissionDataGridGen extends QDataGrid {
		 ...
		 * way of adding a column to this NarroRolePermission datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the NarroRolePermission object, or it can be any QQNode extending from QQN::NarroRolePermission().
		 *
		 ...
		 * sorting using two fields from the NarroRolePermission object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from NarroRolePermission
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= (strpos($strLinkUrl, '?') !== false ? '&' : '?').'intRolePermissionId=<?=urlencode($_ITEM->RolePermissionId)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=urlencode($_ITEM->RolePermissionId)?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->RolePermissionId, false); ?>>' . $strLinkHtml . '</a>';
			$colEditColumn = new QDataGridColumn($strColumnTitle, $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				case 'RolePermissionId': return QQN::NarroRolePermission()->RolePermissionId;
				case 'RoleId': return QQN::NarroRolePermission()->RoleId;
				case 'Role': return QQN::NarroRolePermission()->Role;
				case 'PermissionId': return QQN::NarroRolePermission()->PermissionId;
				case 'Permission': return QQN::NarroRolePermission()->Permission;
				default: throw new QCallerException('Simple Property not found in NarroRolePermissionDataGrid content: ' . $mixContent);
