<?php
	 ...
	 * listing a collection of NarroUser objects.  It includes
	 ...
	 *
	 */
	class NarroUserDataGridGen extends QDataGrid {
		 ...
		 * way of adding a column to this NarroUser datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the NarroUser object, or it can be any QQNode extending from QQN::NarroUser().
		 *
		 ...
		 * sorting using two fields from the NarroUser object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from NarroUser
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= (strpos($strLinkUrl, '?') !== false ? '&' : '?').'intUserId=<?=urlencode($_ITEM->UserId)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=urlencode($_ITEM->UserId)?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->UserId, false); ?>>' . $strLinkHtml . '</a>';
			$colEditColumn = new QDataGridColumn($strColumnTitle, $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				case 'UserId': return QQN::NarroUser()->UserId;
