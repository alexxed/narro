<?php
	 ...
	 * listing a collection of NarroContext objects.  It includes
	 ...
	 *
	 */
	class NarroContextDataGridGen extends QDataGrid {
		 ...
		 * way of adding a column to this NarroContext datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the NarroContext object, or it can be any QQNode extending from QQN::NarroContext().
		 *
		 ...
		 * sorting using two fields from the NarroContext object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from NarroContext
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= (strpos($strLinkUrl, '?') !== false ? '&' : '?').'intContextId=<?=urlencode($_ITEM->ContextId)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=urlencode($_ITEM->ContextId)?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->ContextId, false); ?>>' . $strLinkHtml . '</a>';
			$colEditColumn = new QDataGridColumn($strColumnTitle, $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a NarroContext-based QQNode.
		 * It will also verify that it is a proper NarroContext-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'narro_context') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "narro_context".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'ContextId': return QQN::NarroContext()->ContextId;
				...
				case 'Modified': return QQN::NarroContext()->Modified;
				case 'Active': return QQN::NarroContext()->Active;
				default: throw new QCallerException('Simple Property not found in NarroContextDataGrid content: ' . $mixContent);
