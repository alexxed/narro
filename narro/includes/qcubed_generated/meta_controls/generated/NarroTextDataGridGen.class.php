<?php
	 ...
	 * listing a collection of NarroText objects.  It includes
	 ...
	 *
	 */
	class NarroTextDataGridGen extends QDataGrid {
		 ...
		 * way of adding a column to this NarroText datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the NarroText object, or it can be any QQNode extending from QQN::NarroText().
		 *
		 ...
		 * sorting using two fields from the NarroText object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from NarroText
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= (strpos($strLinkUrl, '?') !== false ? '&' : '?').'intTextId=<?=urlencode($_ITEM->TextId)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=urlencode($_ITEM->TextId)?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->TextId, false); ?>>' . $strLinkHtml . '</a>';
			$colEditColumn = new QDataGridColumn($strColumnTitle, $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				case 'TextId': return QQN::NarroText()->TextId;
				case 'TextValue': return QQN::NarroText()->TextValue;
				case 'TextValueMd5': return QQN::NarroText()->TextValueMd5;
				case 'TextCharCount': return QQN::NarroText()->TextCharCount;
				case 'TextWordCount': return QQN::NarroText()->TextWordCount;
				case 'HasComments': return QQN::NarroText()->HasComments;
				case 'Created': return QQN::NarroText()->Created;
				case 'Modified': return QQN::NarroText()->Modified;
				default: throw new QCallerException('Simple Property not found in NarroTextDataGrid content: ' . $mixContent);
