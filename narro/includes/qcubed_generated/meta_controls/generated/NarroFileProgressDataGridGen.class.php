<?php
	 ...
	 * listing a collection of NarroFileProgress objects.  It includes
	 ...
	 *
	 */
	class NarroFileProgressDataGridGen extends QDataGrid {
		 ...
		 * way of adding a column to this NarroFileProgress datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the NarroFileProgress object, or it can be any QQNode extending from QQN::NarroFileProgress().
		 *
		 ...
		 * sorting using two fields from the NarroFileProgress object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from NarroFileProgress
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= (strpos($strLinkUrl, '?') !== false ? '&' : '?').'intFileProgressId=<?=urlencode($_ITEM->FileProgressId)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=urlencode($_ITEM->FileProgressId)?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->FileProgressId, false); ?>>' . $strLinkHtml . '</a>';
			$colEditColumn = new QDataGridColumn($strColumnTitle, $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a NarroFileProgress-based QQNode.
		 * It will also verify that it is a proper NarroFileProgress-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'narro_file_progress') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "narro_file_progress".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'FileProgressId': return QQN::NarroFileProgress()->FileProgressId;
				case 'FileId': return QQN::NarroFileProgress()->FileId;
				...
				case 'FuzzyTextCount': return QQN::NarroFileProgress()->FuzzyTextCount;
				case 'ProgressPercent': return QQN::NarroFileProgress()->ProgressPercent;
				case 'Export': return QQN::NarroFileProgress()->Export;
				default: throw new QCallerException('Simple Property not found in NarroFileProgressDataGrid content: ' . $mixContent);
