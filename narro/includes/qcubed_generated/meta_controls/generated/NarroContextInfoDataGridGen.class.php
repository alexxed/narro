<?php
	 ...
	 * listing a collection of NarroContextInfo objects.  It includes
	...
	class NarroContextInfoDataGridGen extends QDataGrid {
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->ContextInfoId, false); ?>>' . $strLinkHtml . '</a>';
			$colEditColumn = new QDataGridColumn($strColumnTitle, $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a NarroContextInfo-based QQNode.
		 * It will also verify that it is a proper NarroContextInfo-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'narro_context_info') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "narro_context_info".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'ContextInfoId': return QQN::NarroContextInfo()->ContextInfoId;
				case 'ContextId': return QQN::NarroContextInfo()->ContextId;
				case 'Context': return QQN::NarroContextInfo()->Context;
				case 'LanguageId': return QQN::NarroContextInfo()->LanguageId;
				case 'Language': return QQN::NarroContextInfo()->Language;
				case 'ValidatorUserId': return QQN::NarroContextInfo()->ValidatorUserId;
