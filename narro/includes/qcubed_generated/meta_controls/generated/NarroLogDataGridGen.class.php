<?php
	 ...
	 * listing a collection of NarroLog objects.  It includes
	 ...
	 *
	 */
	class NarroLogDataGridGen extends QDataGrid {
		 ...
		 * way of adding a column to this NarroLog datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the NarroLog object, or it can be any QQNode extending from QQN::NarroLog().
		 *
		 ...
		 * sorting using two fields from the NarroLog object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from NarroLog
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= (strpos($strLinkUrl, '?') !== false ? '&' : '?').'intLogId=<?=urlencode($_ITEM->LogId)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=urlencode($_ITEM->LogId)?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->LogId, false); ?>>' . $strLinkHtml . '</a>';
			$colEditColumn = new QDataGridColumn($strColumnTitle, $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				case 'LogId': return QQN::NarroLog()->LogId;
				case 'LanguageId': return QQN::NarroLog()->LanguageId;
				case 'Language': return QQN::NarroLog()->Language;
				case 'ProjectId': return QQN::NarroLog()->ProjectId;
				case 'Project': return QQN::NarroLog()->Project;
				case 'UserId': return QQN::NarroLog()->UserId;
				case 'User': return QQN::NarroLog()->User;
				case 'Message': return QQN::NarroLog()->Message;
				case 'Priority': return QQN::NarroLog()->Priority;
				case 'Date': return QQN::NarroLog()->Date;
				default: throw new QCallerException('Simple Property not found in NarroLogDataGrid content: ' . $mixContent);
