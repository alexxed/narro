<?php
	 ...
	 * listing a collection of NarroProjectCategory objects.  It includes
	 ...
	 *
	 */
	class NarroProjectCategoryDataGridGen extends QDataGrid {
		 ...
		 * way of adding a column to this NarroProjectCategory datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the NarroProjectCategory object, or it can be any QQNode extending from QQN::NarroProjectCategory().
		 *
		 ...
		 * sorting using two fields from the NarroProjectCategory object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from NarroProjectCategory
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= (strpos($strLinkUrl, '?') !== false ? '&' : '?').'intProjectCategoryId=<?=urlencode($_ITEM->ProjectCategoryId)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=urlencode($_ITEM->ProjectCategoryId)?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->ProjectCategoryId, false); ?>>' . $strLinkHtml . '</a>';
			$colEditColumn = new QDataGridColumn($strColumnTitle, $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a NarroProjectCategory-based QQNode.
		 * It will also verify that it is a proper NarroProjectCategory-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'narro_project_category') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "narro_project_category".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'ProjectCategoryId': return QQN::NarroProjectCategory()->ProjectCategoryId;
				case 'CategoryName': return QQN::NarroProjectCategory()->CategoryName;
				case 'CategoryDescription': return QQN::NarroProjectCategory()->CategoryDescription;
				default: throw new QCallerException('Simple Property not found in NarroProjectCategoryDataGrid content: ' . $mixContent);
