<?php
	 ...
	 * listing a collection of NarroLanguage objects.  It includes
	 ...
	 *
	 */
	class NarroLanguageDataGridGen extends QDataGrid {
		 ...
		 * way of adding a column to this NarroLanguage datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the NarroLanguage object, or it can be any QQNode extending from QQN::NarroLanguage().
		 *
		 ...
		 * sorting using two fields from the NarroLanguage object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from NarroLanguage
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= (strpos($strLinkUrl, '?') !== false ? '&' : '?').'intLanguageId=<?=urlencode($_ITEM->LanguageId)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=urlencode($_ITEM->LanguageId)?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->LanguageId, false); ?>>' . $strLinkHtml . '</a>';
			$colEditColumn = new QDataGridColumn($strColumnTitle, $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				case 'LanguageId': return QQN::NarroLanguage()->LanguageId;
				case 'LanguageName': return QQN::NarroLanguage()->LanguageName;
				case 'LanguageCode': return QQN::NarroLanguage()->LanguageCode;
				case 'CountryCode': return QQN::NarroLanguage()->CountryCode;
				case 'DialectCode': return QQN::NarroLanguage()->DialectCode;
				case 'Encoding': return QQN::NarroLanguage()->Encoding;
				case 'TextDirection': return QQN::NarroLanguage()->TextDirection;
				case 'SpecialCharacters': return QQN::NarroLanguage()->SpecialCharacters;
				case 'PluralForm': return QQN::NarroLanguage()->PluralForm;
				case 'Active': return QQN::NarroLanguage()->Active;
				default: throw new QCallerException('Simple Property not found in NarroLanguageDataGrid content: ' . $mixContent);
