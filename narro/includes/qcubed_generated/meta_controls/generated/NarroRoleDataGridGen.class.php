<?php
	 ...
	 * listing a collection of NarroRole objects.  It includes
	 ...
	 *
	 */
	class NarroRoleDataGridGen extends QDataGrid {
		 ...
		 * in the NarroRole object, or it can be any QQNode extending from QQN::NarroRole().
		 *
		 ...
		 * sorting using two fields from the NarroRole object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from NarroRole
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= (strpos($strLinkUrl, '?') !== false ? '&' : '?').'intRoleId=<?=urlencode($_ITEM->RoleId)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=urlencode($_ITEM->RoleId)?>';
					break;
				default:
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = NarroRole::QueryCount($objConditions);
			}
			...
			// Set the DataSource to be a Query result from NarroRole, given the clauses above
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				case 'RoleId': return QQN::NarroRole()->RoleId;
				case 'RoleName': return QQN::NarroRole()->RoleName;
				default: throw new QCallerException('Simple Property not found in NarroRoleDataGrid content: ' . $mixContent);
