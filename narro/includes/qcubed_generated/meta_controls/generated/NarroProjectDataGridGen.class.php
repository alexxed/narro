<?php
	 ...
	 * listing a collection of NarroProject objects.  It includes
	 ...
	 *
	 */
	class NarroProjectDataGridGen extends QDataGrid {
		 ...
		 * way of adding a column to this NarroProject datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the NarroProject object, or it can be any QQNode extending from QQN::NarroProject().
		 *
		 ...
		 * sorting using two fields from the NarroProject object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from NarroProject
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= (strpos($strLinkUrl, '?') !== false ? '&' : '?').'intProjectId=<?=urlencode($_ITEM->ProjectId)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=urlencode($_ITEM->ProjectId)?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->ProjectId, false); ?>>' . $strLinkHtml . '</a>';
			$colEditColumn = new QDataGridColumn($strColumnTitle, $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				case 'ProjectId': return QQN::NarroProject()->ProjectId;
				case 'ProjectCategoryId': return QQN::NarroProject()->ProjectCategoryId;
				case 'ProjectCategory': return QQN::NarroProject()->ProjectCategory;
				...
				case 'Data': return QQN::NarroProject()->Data;
