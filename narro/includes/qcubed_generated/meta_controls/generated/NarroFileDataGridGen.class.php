<?php
	 ...
	 * listing a collection of NarroFile objects.  It includes
	 ...
	 *
	 */
	class NarroFileDataGridGen extends QDataGrid {
		 ...
		 * way of adding a column to this NarroFile datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the NarroFile object, or it can be any QQNode extending from QQN::NarroFile().
		 *
		 ...
		 * sorting using two fields from the NarroFile object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from NarroFile
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= (strpos($strLinkUrl, '?') !== false ? '&' : '?').'intFileId=<?=urlencode($_ITEM->FileId)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=urlencode($_ITEM->FileId)?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->FileId, false); ?>>' . $strLinkHtml . '</a>';
			$colEditColumn = new QDataGridColumn($strColumnTitle, $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a NarroFile-based QQNode.
		 * It will also verify that it is a proper NarroFile-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'narro_file') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "narro_file".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'FileId': return QQN::NarroFile()->FileId;
				case 'FileName': return QQN::NarroFile()->FileName;
				case 'FilePath': return QQN::NarroFile()->FilePath;
				case 'FileMd5': return QQN::NarroFile()->FileMd5;
				case 'ParentId': return QQN::NarroFile()->ParentId;
				case 'Parent': return QQN::NarroFile()->Parent;
				case 'TypeId': return QQN::NarroFile()->TypeId;
				case 'ProjectId': return QQN::NarroFile()->ProjectId;
				case 'Project': return QQN::NarroFile()->Project;
