<?php
	 ...
	 * listing a collection of NarroProjectProgress objects.  It includes
	 ...
	 *
	 */
	class NarroProjectProgressDataGridGen extends QDataGrid {
		 ...
		 * way of adding a column to this NarroProjectProgress datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the NarroProjectProgress object, or it can be any QQNode extending from QQN::NarroProjectProgress().
		 *
		 ...
		 * sorting using two fields from the NarroProjectProgress object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from NarroProjectProgress
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= (strpos($strLinkUrl, '?') !== false ? '&' : '?').'intProjectProgressId=<?=urlencode($_ITEM->ProjectProgressId)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=urlencode($_ITEM->ProjectProgressId)?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->ProjectProgressId, false); ?>>' . $strLinkHtml . '</a>';
			$colEditColumn = new QDataGridColumn($strColumnTitle, $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a NarroProjectProgress-based QQNode.
		 * It will also verify that it is a proper NarroProjectProgress-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'narro_project_progress') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "narro_project_progress".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'ProjectProgressId': return QQN::NarroProjectProgress()->ProjectProgressId;
				case 'ProjectId': return QQN::NarroProjectProgress()->ProjectId;
