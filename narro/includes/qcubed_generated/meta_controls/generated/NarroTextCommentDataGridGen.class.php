<?php
	 ...
	 * listing a collection of NarroTextComment objects.  It includes
	 ...
	 *
	 */
	class NarroTextCommentDataGridGen extends QDataGrid {
		 ...
		 * way of adding a column to this NarroTextComment datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the NarroTextComment object, or it can be any QQNode extending from QQN::NarroTextComment().
		 *
		 ...
		 * sorting using two fields from the NarroTextComment object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from NarroTextComment
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= (strpos($strLinkUrl, '?') !== false ? '&' : '?').'intTextCommentId=<?=urlencode($_ITEM->TextCommentId)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=urlencode($_ITEM->TextCommentId)?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->TextCommentId, false); ?>>' . $strLinkHtml . '</a>';
			$colEditColumn = new QDataGridColumn($strColumnTitle, $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				case 'TextCommentId': return QQN::NarroTextComment()->TextCommentId;
				case 'TextId': return QQN::NarroTextComment()->TextId;
				case 'Text': return QQN::NarroTextComment()->Text;
				case 'UserId': return QQN::NarroTextComment()->UserId;
				case 'User': return QQN::NarroTextComment()->User;
				case 'LanguageId': return QQN::NarroTextComment()->LanguageId;
				case 'Language': return QQN::NarroTextComment()->Language;
				case 'Created': return QQN::NarroTextComment()->Created;
				case 'Modified': return QQN::NarroTextComment()->Modified;
				case 'CommentText': return QQN::NarroTextComment()->CommentText;
				case 'CommentTextMd5': return QQN::NarroTextComment()->CommentTextMd5;
				default: throw new QCallerException('Simple Property not found in NarroTextCommentDataGrid content: ' . $mixContent);
