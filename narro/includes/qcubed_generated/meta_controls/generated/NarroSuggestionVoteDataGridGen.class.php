<?php
	 ...
	 * listing a collection of NarroSuggestionVote objects.  It includes
	 ...
	 *
	 */
	class NarroSuggestionVoteDataGridGen extends QDataGrid {
		 ...
		 * way of adding a column to this NarroSuggestionVote datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the NarroSuggestionVote object, or it can be any QQNode extending from QQN::NarroSuggestionVote().
		 *
		 ...
		 * sorting using two fields from the NarroSuggestionVote object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from NarroSuggestionVote
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= (strpos($strLinkUrl, '?') !== false ? '&' : '?').'intSuggestionId=<?=urlencode($_ITEM->SuggestionId)?>&intContextId=<?=urlencode($_ITEM->ContextId)?>&intUserId=<?=urlencode($_ITEM->UserId)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=urlencode($_ITEM->SuggestionId)?>/<?=urlencode($_ITEM->ContextId)?>/<?=urlencode($_ITEM->UserId)?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->SuggestionId . "," . $_ITEM->ContextId . "," . $_ITEM->UserId, false); ?>>' . $strLinkHtml . '</a>';
			$colEditColumn = new QDataGridColumn($strColumnTitle, $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				case 'SuggestionId': return QQN::NarroSuggestionVote()->SuggestionId;
				case 'Suggestion': return QQN::NarroSuggestionVote()->Suggestion;
				case 'ContextId': return QQN::NarroSuggestionVote()->ContextId;
				case 'Context': return QQN::NarroSuggestionVote()->Context;
				case 'UserId': return QQN::NarroSuggestionVote()->UserId;
				case 'User': return QQN::NarroSuggestionVote()->User;
				case 'VoteValue': return QQN::NarroSuggestionVote()->VoteValue;
				case 'Created': return QQN::NarroSuggestionVote()->Created;
				case 'Modified': return QQN::NarroSuggestionVote()->Modified;
				default: throw new QCallerException('Simple Property not found in NarroSuggestionVoteDataGrid content: ' . $mixContent);
