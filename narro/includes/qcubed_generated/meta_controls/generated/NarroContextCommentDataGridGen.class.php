<?php
	 ...
	 * listing a collection of NarroContextComment objects.  It includes
	 ...
	 *
	 */
	class NarroContextCommentDataGridGen extends QDataGrid {
		 ...
		 * way of adding a column to this NarroContextComment datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the NarroContextComment object, or it can be any QQNode extending from QQN::NarroContextComment().
		 *
		 ...
		 * sorting using two fields from the NarroContextComment object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from NarroContextComment
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = NarroContextComment::QueryCount($objConditions);
			}
			...
			// Set the DataSource to be a Query result from NarroContextComment, given the clauses above
			$this->DataSource = NarroContextComment::QueryArray($objConditions, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a NarroContextComment-based QQNode.
		 * It will also verify that it is a proper NarroContextComment-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'narro_context_comment') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "narro_context_comment".');
				...
				case 'CommentTextMd5': return QQN::NarroContextComment()->CommentTextMd5;
				default: throw new QCallerException('Simple Property not found in NarroContextCommentDataGrid content: ' . $mixContent);
