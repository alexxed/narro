<?php
	 ...
	 * listing a collection of NarroSuggestionComment objects.  It includes
	 ...
	 *
	 */
	class NarroSuggestionCommentDataGridGen extends QDataGrid {
		 ...
		 * way of adding a column to this NarroSuggestionComment datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the NarroSuggestionComment object, or it can be any QQNode extending from QQN::NarroSuggestionComment().
		 *
		 ...
		 * sorting using two fields from the NarroSuggestionComment object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from NarroSuggestionComment
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= (strpos($strLinkUrl, '?') !== false ? '&' : '?').'intCommentId=<?=urlencode($_ITEM->CommentId)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=urlencode($_ITEM->CommentId)?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->CommentId, false); ?>>' . $strLinkHtml . '</a>';
			$colEditColumn = new QDataGridColumn($strColumnTitle, $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = NarroSuggestionComment::QueryCount($objConditions);
			}
			...
			// Set the DataSource to be a Query result from NarroSuggestionComment, given the clauses above
			$this->DataSource = NarroSuggestionComment::QueryArray($objConditions, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a NarroSuggestionComment-based QQNode.
		 * It will also verify that it is a proper NarroSuggestionComment-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'narro_suggestion_comment') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "narro_suggestion_comment".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'CommentId': return QQN::NarroSuggestionComment()->CommentId;
				case 'SuggestionId': return QQN::NarroSuggestionComment()->SuggestionId;
				case 'Suggestion': return QQN::NarroSuggestionComment()->Suggestion;
				case 'UserId': return QQN::NarroSuggestionComment()->UserId;
				case 'User': return QQN::NarroSuggestionComment()->User;
				case 'CommentText': return QQN::NarroSuggestionComment()->CommentText;
				case 'CommentTextMd5': return QQN::NarroSuggestionComment()->CommentTextMd5;
				case 'Created': return QQN::NarroSuggestionComment()->Created;
				case 'Modified': return QQN::NarroSuggestionComment()->Modified;
				default: throw new QCallerException('Simple Property not found in NarroSuggestionCommentDataGrid content: ' . $mixContent);
