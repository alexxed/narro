<?php
	 ...
	 * listing a collection of NarroPermission objects.  It includes
	 ...
	 *
	 */
	class NarroPermissionDataGridGen extends QDataGrid {
		 ...
		 * way of adding a column to this NarroPermission datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the NarroPermission object, or it can be any QQNode extending from QQN::NarroPermission().
		 *
		 ...
		 * sorting using two fields from the NarroPermission object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from NarroPermission
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= (strpos($strLinkUrl, '?') !== false ? '&' : '?').'intPermissionId=<?=urlencode($_ITEM->PermissionId)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=urlencode($_ITEM->PermissionId)?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->PermissionId, false); ?>>' . $strLinkHtml . '</a>';
			$colEditColumn = new QDataGridColumn($strColumnTitle, $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a NarroPermission-based QQNode.
		 * It will also verify that it is a proper NarroPermission-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'narro_permission') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "narro_permission".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'PermissionId': return QQN::NarroPermission()->PermissionId;
				case 'PermissionName': return QQN::NarroPermission()->PermissionName;
				default: throw new QCallerException('Simple Property not found in NarroPermissionDataGrid content: ' . $mixContent);
