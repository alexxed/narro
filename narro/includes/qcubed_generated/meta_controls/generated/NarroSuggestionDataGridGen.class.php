<?php
	 ...
	 * listing a collection of NarroSuggestion objects.  It includes
	 ...
	 *
	 */
	class NarroSuggestionDataGridGen extends QDataGrid {
		 ...
		 * way of adding a column to this NarroSuggestion datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the NarroSuggestion object, or it can be any QQNode extending from QQN::NarroSuggestion().
		 *
		 ...
		 * sorting using two fields from the NarroSuggestion object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from NarroSuggestion
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= (strpos($strLinkUrl, '?') !== false ? '&' : '?').'intSuggestionId=<?=urlencode($_ITEM->SuggestionId)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=urlencode($_ITEM->SuggestionId)?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->SuggestionId, false); ?>>' . $strLinkHtml . '</a>';
			$colEditColumn = new QDataGridColumn($strColumnTitle, $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				case 'SuggestionId': return QQN::NarroSuggestion()->SuggestionId;
				case 'UserId': return QQN::NarroSuggestion()->UserId;
				case 'User': return QQN::NarroSuggestion()->User;
				case 'TextId': return QQN::NarroSuggestion()->TextId;
				case 'Text': return QQN::NarroSuggestion()->Text;
				case 'LanguageId': return QQN::NarroSuggestion()->LanguageId;
				case 'Language': return QQN::NarroSuggestion()->Language;
				case 'SuggestionValue': return QQN::NarroSuggestion()->SuggestionValue;
				case 'SuggestionValueMd5': return QQN::NarroSuggestion()->SuggestionValueMd5;
				case 'SuggestionCharCount': return QQN::NarroSuggestion()->SuggestionCharCount;
				case 'SuggestionWordCount': return QQN::NarroSuggestion()->SuggestionWordCount;
				case 'HasComments': return QQN::NarroSuggestion()->HasComments;
				case 'IsImported': return QQN::NarroSuggestion()->IsImported;
				case 'Created': return QQN::NarroSuggestion()->Created;
				case 'Modified': return QQN::NarroSuggestion()->Modified;
				default: throw new QCallerException('Simple Property not found in NarroSuggestionDataGrid content: ' . $mixContent);
